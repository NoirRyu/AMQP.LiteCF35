...
namespace Amqp
{
    ...
    public abstract class AmqpObject
    {
        ...
        //chris public void Close(int waitUntilEnded = DefaultCloseTimeout, Error error = null)
        public void Close(int waitUntilEnded , Error error )
        {
            ...
            if (!this.OnClose(error))
            {
                if (waitUntilEnded > 0)
                {
                    //chris this.endEvent.WaitOne(waitUntilEnded);
        ...
        public void Close()
        {
            Close(DefaultCloseTimeout, null);
        }
